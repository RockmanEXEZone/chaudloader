...
pub fn new<'a>(
    ...
    table.set(
        "write_dat_contents",
        table.get::<_, mlua::Value>(lua.create_string("write_zipdat_contents")?)?,
    )?;
    table.set(
        "read_dat_contents",
        table.get::<_, mlua::Value>(lua.create_string("read_zipdat_contents")?)?,
    )?;
