...
impl HandleTracker {
    fn insert(&mut self, path: &std::path::Path, handle: winapi::shared::ntdef::HANDLE) -> usize {
    ...
    fn dupe(
        ...
        let handles = self.path_to_handles.get_mut(&path).unwrap();
        handles.insert(dest_handle);
        handles.len()
    }
...
fn as_ntapi_unicode_string(buf: &mut [u16]) -> winapi::shared::ntdef::UNICODE_STRING {
    let byte_len = buf.len() * std::mem::size_of::<winapi::shared::ntdef::WCHAR>();
    winapi::shared::ntdef::UNICODE_STRING {
        Buffer: buf.as_mut_ptr(),
        Length: byte_len as winapi::shared::ntdef::USHORT,
        MaximumLength: byte_len as winapi::shared::ntdef::USHORT,
    }
}

unsafe fn osstring_from_ntapi_unicode_string(
    ustr: &winapi::shared::ntdef::UNICODE_STRING,
) -> std::ffi::OsString {
    std::ffi::OsString::from_wide(std::slice::from_raw_parts(
        ustr.Buffer,
        ustr.Length as usize / std::mem::size_of::<winapi::shared::ntdef::WCHAR>(),
    ))
}
...
unsafe fn on_nt_create_file(
    ...
    let object_name = osstring_from_ntapi_unicode_string(&*(*object_attributes).ObjectName);
    ...
    let status = NtCreateFileHook.call(
        ...
        &mut winapi::shared::ntdef::OBJECT_ATTRIBUTES {
            ...
            ObjectName: &mut as_ntapi_unicode_string(&mut {
                // Path needs to be converted into an NT Object Manager path (\??\...).
                //
                // This is not the same as a UNC path (\\?\...), confusingly.
    ...
    let refcount = handle_tracker.insert(path, handle);
    log::info!(
...
unsafe fn on_duplicate_handle(
    ...
    if handle_tracker.dupe(h_source_handle, target_handle) > 0 {
        log::info!(
...
unsafe fn on_close_handle(
    ...
    if let Some((_path, _)) = handle_tracker
        .remove(h_object)
        .filter(|(_, refcount)| *refcount == 0)
    {
        // FIXME: We would like to purge assets once the last handle is closed, but unfortunately BNLC leaks a handle to dat archives.
        // I'm not sure if there is really a way around this, unfortunately.
